...
HRESULT UpdateFrame()
{
	...
	if (m_aPlayers[0].bAlive) {
		if (m_aPlayers[0].bShooting) m_aPlayers[0].model.Draw(&rifle, &fire);
		else                         m_aPlayers[0].model.Draw(&rifle);
	} else {
	...
	for (int i = 1; i < g_iNumPlayers; i++) {
		if (m_aPlayers[i].bAlive) {
			...
			if (m_aPlayers[i].bShooting) m_aPlayers[i].model.Draw(&rifle, &fire);
			else                         m_aPlayers[i].model.Draw(&rifle);
		} else {
