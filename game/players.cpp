...
const char* szMeshFile[] = {
    {"data/meshes/ragdoll/head_mesh_c.dat"},     {"data/meshes/ragdoll/body_mesh_c.dat"},
    {"data/meshes/ragdoll/belt_mesh_c.dat"},     {"data/meshes/ragdoll/shoulder_mesh_c.dat"},
    {"data/meshes/ragdoll/shoulder_mesh_c.dat"}, {"data/meshes/ragdoll/arm_mesh_c.dat"},
    {"data/meshes/ragdoll/arm_mesh_c.dat"},      {"data/meshes/ragdoll/hand_mesh_c.dat"},
    {"data/meshes/ragdoll/hand_mesh_c.dat"},     {"data/meshes/ragdoll/thigh_mesh_c.dat"},
    {"data/meshes/ragdoll/thigh_mesh_c.dat"},    {"data/meshes/ragdoll/leg_mesh_c.dat"},
    {"data/meshes/ragdoll/leg_mesh_c.dat"},      {"data/meshes/ragdoll/foot_mesh_c.dat"},
...
HRESULT PLAYER::Init(SDL_Renderer* pRenderer, const char* szModelFileName,
                     const char* szBodyFileName, const char* szRagDollDir)
{
	bAlive = true;
	...
	for (int i = 0; i < NUM_PARTS; i++)
	{
		...
		float dx = (model.GetPart(i)->GetRotationX() - (model.GetPart(i)->iWidth >> 1)) * 0.4f;
		float dy = (model.GetPart(i)->GetRotationY() - (model.GetPart(i)->iHeight >> 1)) * 0.4f;
...
void PLAYER::Update(DWORD dwTime, bool* actions)
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		if (bActiveGrenade)
		{
			for (int i = 0; i < g_iNumWalls; i++)
				if (grenade_body.Collide(aWalls[i], MTD, t))
					grenade_body.ResolveCollision(aWalls[i], MTD, t);

			for (int i = 0; i < g_iNumBodies; i++)
				if (grenade_body.Collide(aBodies[i], MTD, t))
					grenade_body.ResolveCollision(aBodies[i], MTD, t);

			for (int i = 0; i < g_iNumPlayers; i++)
			{
				if (m_aPlayers[i].bAlive)
					if (grenade_body.Collide(m_aPlayers[i].body, MTD, t))
					{
						DWORD d = tmGrenade.Delta();
						if (d < 2000)
						{
							tmGrenade.LastTickCount -= 2000 - d;
						}
					}
			}

			grenade_body.ApplyForce(VECTOR2D(0.0f, g * m_aPlayers[0].grenade_body.fMass * 0.5f));
			...
			if (tmGrenade.Delta() > 2000)
			{
				...
				for (int i = 0; i < g_iNumBodies; i++)
				{
					if (CircleIntersect(aBodies[i], grenade_body.Pos, EXPLODE_RANGE, t, MTD))
					{
						float fForce = EXPLODE_FORCE * (t / EXPLODE_RANGE);
						VECTOR2D force_point = MTD * (EXPLODE_RANGE - t) + grenade_body.Pos;
						aBodies[i].ApplyImpulse(fForce, MTD, force_point);
					}
				}

				for (int i = 0; i < g_iNumPlayers; i++)
				{
					if (CircleIntersect(m_aPlayers[i].body, grenade_body.Pos, EXPLODE_RANGE, t,
					                    MTD))
					{
						float fForce = EXPLODE_FORCE * (t / EXPLODE_RANGE);
						VECTOR2D force_point = MTD * (EXPLODE_RANGE - t) + grenade_body.Pos;
						m_aPlayers[i].body.ApplyImpulse(fForce, MTD, force_point);
	...
	if (bCollided)
	{
		...
		if (body.Velocity.y > FALL_MINVEL)
			health -= (int)((body.Velocity.y / FALL_MAXVEL) * (float)c_NumHealth);
	}
	...
	while (j < g_iNumBodies)
	{
		...
		{
			body.ResolveCollision(aBodies[j], MTD, t);
			body.fAngVelocity = 0.0f;
			Axis += MTD * t;
			bCollided = true;
			coll = COLLISION::HORZ;
	...
	*/
	...
	if (actions[static_cast<int>(ACTION::MOVELEFT)])
	{
		...
		else if (body.Velocity.x > -FLY_MAXVEL)
	    ...
	    ammo[static_cast<int>(WEAPON::RIFLE)] > 0)
	{
		...
		if (pViewObject)
			...
			{
				pViewObject->ApplyImpulse(-SHOOT_FORCE, minNt, view_pt);
	...
	if (actions[static_cast<int>(ACTION::ALTSHOOT)] && tmAltShoot.Delta() > 2500 &&
	    ammo[static_cast<int>(WEAPON::GRENADE)] > 0)
	{
		// init grenade
		if (DotProduct(cursor, cursor) > 2500)
			grenade_body.Move(-grenade_body.Pos + body.Pos + Normalize(cursor) * 50);
		else
			grenade_body.Move(-grenade_body.Pos + body.Pos);
...
HRESULT AddPlayer()
{
	...
	HRESULT hr;
	hr = m_aPlayers[index].Init(g_renderer, "data/models/soldat.m2d", "data/meshes/body.dat",
	                            "data/meshes/ragdoll/");
	if (FAILED(hr))
	{
	...
	m_aPlayers[index].model.SetAnimation(
	    &animations[static_cast<int>(ANIMATION_TYPE::ANIMATION_IDLE)]);
