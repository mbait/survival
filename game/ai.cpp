...
void GetAIActions(int index, bool *actions)
{
	...
	int ammo_level = m_aPlayers[index].ammo[static_cast<int>(WEAPON::RIFLE)] * 100
	               / PLAYER::c_NumRifleAmmo;
	int grenade_level = m_aPlayers[index].ammo[static_cast<int>(WEAPON::GRENADE)] * 100
	                  / PLAYER::c_NumGrenades;
				...
				for (int i = 0; i < g_iNumPackPlaces; i++) {
					if (aPacks[i].bActive
					 && apPathDistance[src_vert][aPacks[i].nVertexIndex] < min) {
						...
						case PACK_TYPE::PACK_AMMO:    if (ammo_level > 60)     continue; break;
						case PACK_TYPE::PACK_GRENADE: if (grenade_level==100)  continue; break;
						case PACK_TYPE::PACK_HEALTH:  if (health_level > 40)   continue; break;
...
int ScoreCmp(const void* arg_1, const void* arg_2)
{
	int a = *(int*)arg_1;
	int b = *(int*)arg_2;
	if (m_aDeath[a] < m_aDeath[b]) return -1;
	if (m_aDeath[a] > m_aDeath[b]) return 1;
	if (m_aFrags[a] > m_aFrags[b]) return -1;
	if (m_aFrags[a] < m_aFrags[b]) return 1;
	return 0;
}
