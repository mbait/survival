...
struct VECTOR2D
{
	...
	constexpr VECTOR2D& operator+=(VECTOR2D const& A) noexcept
	{
		x += A.x;
		y += A.y;
		return *this;
	...
	constexpr VECTOR2D& operator*=(float fM) noexcept
	{
		x *= fM;
		y *= fM;
		return *this;
	}
	constexpr VECTOR2D& operator/=(float fD) noexcept
	{
		x /= fD;
		y /= fD;
		return *this;
...
public:
	...
	[[nodiscard]] constexpr float operator()(int i, int j) const noexcept
	{
		return e[i][j];
	}
	[[nodiscard]] constexpr float& operator()(int i, int j) noexcept
	{
		return e[i][j];
	}
	...
	{
		return MATRIX(e11 * M.e11 + e12 * M.e21, e11 * M.e12 + e12 * M.e22,
		              e21 * M.e11 + e22 * M.e21, e21 * M.e12 + e22 * M.e22);
	}

	[[nodiscard]] constexpr MATRIX operator^(const MATRIX& M) const noexcept
	{
		return MATRIX(e11 * M.e11 + e12 * M.e12, e11 * M.e21 + e12 * M.e22,
		              e21 * M.e11 + e22 * M.e12, e21 * M.e21 + e22 * M.e22);
	}
...
[[nodiscard]] constexpr float DegToRad(float fDeg) noexcept
{
	return fDeg / 180.0f * Pi;
}
[[nodiscard]] constexpr float RadToDeg(float fRad) noexcept
{
	return fRad / Pi * 180.0f;
}
