...
VECTOR2D::VECTOR2D(float fTheta) noexcept : x(std::cos(fTheta)), y(std::sin(fTheta)) {}
...
{
	return {A.x * M.e11 + A.y * M.e12, A.x * M.e21 + A.y * M.e22};
}

VECTOR2D operator^(VECTOR2D const& A, MATRIX const& M) noexcept
{
	return {A.x * M.e11 + A.y * M.e21, A.x * M.e12 + A.y * M.e22};
}
