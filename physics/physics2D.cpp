...
bool RIGIDBODY::Collide(RIGIDBODY &body, VECTOR2D &MTD, float &t)
{
	...
	//quick rejection
	VECTOR2D v_mina(_HUGE, _HUGE), v_maxa(-_HUGE, -_HUGE),
		v_minb(_HUGE, _HUGE), v_maxb(-_HUGE, -_HUGE);
...
bool CircleIntersect(RIGIDBODY &body, VECTOR2D center, 
					 float fRadius, float &t, VECTOR2D &Nt)
{
...
float SolveContact(RIGIDBODY &A, RIGIDBODY &B, VECTOR2D N, VECTOR2D P, float t)
{
	...
	float Denom = DotProduct(Normal, Normal)*(AntiMa+AntiMb)+pda*pda*AntiIa+pdb*pdb*AntiIb;
	//----!!!!!!!----//
	float fImpulse;
	if(Denom)
		fImpulse = Num/Denom;
...
void JOINT::CalcForce()
{
	...
	VECTOR2D spring_vec  = aBodies[1]->Pos-aBodies[0]->Pos
		+P1-P0;
	...
	aBodies[0]->Force    -= spring_force;
	aBodies[0]->fTorque  -= PerpDotProduct(P0, 
		spring_force);
	aBodies[1]->Force	 += spring_force;
	aBodies[1]->fTorque  += PerpDotProduct(P1,
		spring_force);
