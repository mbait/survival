...
bool PARTICLE::Update(DWORD dwTime, VECTOR2D vGravity)
{
	...
	color_current.Alpha +=
	    (int)(color_end.Alpha - color_start.Alpha) * ((float)counter / (float)TTL);

	color_current.R += (int)(color_end.R - color_start.R) * ((float)counter / (float)TTL);

	color_current.G += (int)(color_end.G - color_start.G) * ((float)counter / (float)TTL);
