...
void compose_dst(float px, float py, float pivot_x, float pivot_y, int w, int h, float scale,
...
HRESULT finish_init(SDL_Renderer* renderer, SDL_Texture* tex, SDL_Texture*& out_tex,
                    SDL_Renderer*& out_renderer, int& out_w, int& out_h, float& out_pivot_x,
                    float& out_pivot_y)
{
	if (!tex)
	{
...
HRESULT SPRITE::Init(SDL_Renderer* renderer, const char* szFileName)
{
	...
	return finish_init(renderer, tex, pTexture, pRenderer, iWidth, iHeight, fRotationX, fRotationY);
}
...
HRESULT SPRITE::Draw(BYTE Alpha)
{
	...
	SDL_Rect dst;
	SDL_Point center;
	compose_dst(fxPos, fyPos, fRotationX, fRotationY, iWidth, iHeight, fScale, dst, center);
	...
	if (SDL_RenderCopyEx(pRenderer, pTexture, nullptr, &dst, to_sdl_angle_deg(angle), &center,
	                     flip) != 0)
	{
		return E_FAIL;
	}
...
HRESULT SPRITE::Draw(const Affine2D& parent, BYTE Alpha)
{
	...
	SDL_Rect dst;
	SDL_Point center;
	compose_dst(world.tx, world.ty, fRotationX, fRotationY, iWidth, iHeight, world.scale, dst,
	            center);
...
HRESULT SPRITE::Draw(float fX, float fY, float fR, float fRX, float fRY, float fS, BYTE Alpha)
{
	if (!pRenderer || !pTexture)
	{
		return E_FAIL;
