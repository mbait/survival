...
int main(int /*argc*/, char* argv[])
{
	...
	std::fprintf(stderr, "settings: %dx%d %s, refresh=%d, AA=%d, map=%s (from %s)\n",
	             g_iScreenWidth, g_iScreenHeight, g_bFullScreen ? "fullscreen" : "windowed",
	...
	const Uint32 window_flags =
	    SDL_WINDOW_SHOWN | (g_bFullScreen ? Uint32 {SDL_WINDOW_FULLSCREEN_DESKTOP} : Uint32 {0});

	SDL_Window* window =
	    SDL_CreateWindow("survival", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, g_iScreenWidth,
	                     g_iScreenHeight, window_flags);
	if (!window)
	{
	...
	{
		const std::filesystem::path map_path = std::filesystem::path("data/maps") / g_szMapName;
